...
public class JSONTokenizer implements Tokenizer<Recognizer<Token, String>, Expression<String>> {
    ...
    public List<Token.Entry> tokenize(CharSequence sequence) {
        ...
        while (matcher.find()) {
            ...
            tokens.add(new Token.EntryImplementation(token.get(), value, position, ++ordinal));
        }
    ...
    private void ensureExpressions() {
        if (expressions.isEmpty()) {
            throw new JSONTokenizerException("NO EXPRESSIONS CONFIGURED");
        } else {
    ...
    private void ensureRecognizers() {
        if (expressions.isEmpty()) {
            throw new JSONTokenizerException("NO RECOGNIZERS CONFIGURED");
        } else {
